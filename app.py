...
@st.cache_resource(show_spinner="Loading model … ⚙️")
def get_model():
    ...
    if not user_input.strip():
        ...
        with st.spinner("Analyzing your brain rot level … 🧬"):
            try:
                ...
                label = run_inference(user_input, tokenizer, model)
                result = parse_output(label)
        ...
        st.markdown(
            f"""<div style="background:#1f2937; border-radius:9999px;
        ...
        if result["advice"]:
            ...
            for tip in result["advice"]:
                st.markdown(
                    f"<div class='advice-card' style='border-color:{bar_color};'>✦ {tip}</div>",
                    unsafe_allow_html=True,
                )
